...
Like mentioned in \ref{roi}, the group was just interested in the horizontal position of the persons that would step into the shot of the camera. That was why the group just had on strip of LEDs where persons should be detected. For the image processing program that also meant that just this part of the picture had to be analysed. This also meant that the program could run much faster, when just a little part of the picture had to be analysed. This little part is called the region of interest or short the ROI. 
...
\section{Morphology}
Because the group arranged the LEDs on the LED strip in an interval of 4 centimetres there were some light gaps on the taken picture. This meant that some of the pixels that actually should have been detected as change, were detected as unchanged. This caused the program to detect some persons doubled, because they were separated even though they were one person. 
The group solved this by applying morphology to the picture. We used \textit{Closing} to close the caps within the persons. Closing consist of first dilating the picture and then eroding it.
\begin{lstlisting}
...
\subsection{Take Background picture}\label{backgroundConfig}
...
\end{lstlisting}
This function works just on small blobs. The problem was that for each pixel a new function was called causing a stack overflow. That is why they had to make a new function that wouldn't call more and more functions. The group wrote the logic behind the recursive function down and analysed what it was doing. They were able to reformulate its logic to a function that wasn't recursive, but did the same thing just in a while loop with eight cases for where the function will look next. The first four cases are the ones where the function goes forward to the next pixel, with a priority for each direction. The priorities five to eight are for when the function already had tried to move forward and the only possibility is to go back. When non of these cases occur, the function is done with the analysis and the while loop is exited.
\begin{lstlisting}
...
When trying to find new persons the group limited the search to looking at the leftmost and the right most pixels since nobody could come from above or from the underneath the camera. Since they also limited the their previous image processing to a region of interest, they also just had to look for new persons in this area. They actually decided just to look at the brightest pixel, where hopefully the LED strip was positioned. The brightest points in the background were already found in the \textbf{configBG()} function described earlier. 
\begin{lstlisting}
